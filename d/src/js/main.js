//= plugins.js
	...
	PanoramaDrag.prototype._draggableInit = function () {
		var self = this;
		self.interact = interact(this.phoneSelector)
		.draggable({
			inertia: true,
			restrict: {
				restriction: "parent",
  ...
  PanoramaAnimator.prototype._firstStageIn = function () {
    ...
    pfsi
    .staggerTo($('.v-line, .v-side-line'), .4, {scale: 1})
    .to($('.h-line'), .5, {scale: 1}, '-=.2')
    .staggerTo($('.circle-big, .circle-small'), .3, {opacity: 1},.2, "-=.5")
    .fromTo($('.circle-big__title .title1'), .4, {x: "-30"}, {x: 0, opacity: 1})
    .fromTo($('.circle-big__title .title2'), .4, {x: "30"}, {x: 0, opacity: 1}, "-=.1")
    .to($('.phone'), .6, {opacity: 1})
    .fromTo($('.info-text'), .6, {y: "30"}, {y: 0, opacity: 1}, "-=.5")
    .to($('.rotate-icon'), .8, {opacity: 1}, "-=.3")
  };
  ...
  PanoramaAnimator.prototype._firstStageOut = function () {
   ...
   pfso
   .staggerTo($('.v-line, .v-side-line, .h-line'), .3, {scale: 0})
   .staggerTo($('.circle-big, .circle-small'), .3, {opacity: 0})
   .staggerTo($('.phone, .info-text, .rotate-icon'), .4, {opacity: 0})
   .to($('.circle-big__title .title1'), .3, {x: 105, opacity: 0})
   .to($('.circle-big__title .title2'), .3, {x: -150, opacity: 0}, "-=.3")
 };
