...
$(document).ready(function() {
	...
	var PanoramaDrag = function (phoneSelector, panoramaSelector, animationStartDelay, mobileSlowness) {
		...
		this.MOBILE_SLOWNESS = mobileSlowness || 2;
		this.x = 0;
	};
	...
	PanoramaDrag.prototype._draggableInit = function () {
		...
		draggie.on( 'dragStart', self._dragStartEventListener.bind(self));
		draggie.on( 'dragMove', self._dragMoveEventListener.bind(self));
		draggie.on( 'dragEnd', self._dragEndEventListener.bind(self));

		// interact(this.phoneSelector)
		// .draggable({
		// 	inertia: true,
		// 	restrict: {
		// 		restriction: "parent",
	...
	PanoramaDrag.prototype._dragMoveEventListener = function (event, pointer, moveVector) {
		// this._changePhonePosition(event, pointer, moveVector);
		this._changePanoramaPosition(event, pointer, moveVector);
	};
	PanoramaDrag.prototype._dragEndEventListener = function () {};

	PanoramaDrag.prototype._changePhonePosition = function (event, pointer, moveVector) {
	...
	PanoramaDrag.prototype._changePanoramaPosition = function (event, pointer, moveVector) {

		var target = this._panorama,
				dx = this._getDx(moveVector),
				windowW = window.innerWidth,
				targetW = target.getBoundingClientRect().width,
				x = (parseFloat(target.getAttribute('data-x')) || 0) + dx * -1 * (this.DRAG_INCREMENT/10);
console.log(x);
console.log(dx);
		if (dx < 1 && x > windowW/2 * -1) { 
    	// Левая граница блока панорамы = левой границе окна
    	x = windowW/2 * -1;
    }

    if (dx > 1 && x < windowW/2 - targetW) { 
  ...
  PanoramaDrag.prototype._getDx = function (moveVector) {
		this.x = moveVector.x - this.x;
		return this.x;
  };

var newPanorama = new PanoramaDrag('.phone.draggable', '.panorama', 4000);
